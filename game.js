...
let arenaBg = null;
    ...
    function updatePlayerPhysics(player, controls, joyVec) {
        ...
        const dir = Math.max(-1, Math.min(1, (keys[controls.rotR] ? 1 : 0) - (keys[controls.rotL] ? 1 : 0) + (player.rotBtn || 0)));
    ...
    function updatePhysics() {
        if (!isGameRunning) return;
        updatePlayerPhysics(p1, { up: 'KeyW', down: 'KeyS', left: 'KeyA', right: 'KeyD', rotL: 'KeyQ', rotR: 'KeyE' }, j1Vec);
        if (gameMode === '1v1') {
            updatePlayerPhysics(p2, { up: 'ArrowUp', down: 'ArrowDown', left: 'ArrowLeft', right: 'ArrowRight', rotL: 'Comma', rotR: 'Period' }, j2Vec);
            separateRobots();
        }
        updateProjectiles();
    }
